...
class FairnessAttack:
    def __init__(
        self, K: int, X: np.ndarray, S: np.ndarray, min_dist_init: float = 0.5
    ) -> None:
        self.K = K
        self.X = X
        self.S = S
    ...
    def get_initial_clusters(self):
        ...
        benign_clustering = ClusteringKMedoid("normal", self.K, self.X, self.S)
        (
            self.initial_centers,
            self.initial_centers_idx,
            self.initial_members,
            _,
        ) = benign_clustering.get_centers()
        _, self.pre_attack_utility = benign_clustering.calculate_fairness_utility(
            self.initial_centers_idx
        )
    ...
    def get_final_clusters(self):
        ...
        final_clustering = ClusteringKMedoid("normal", self.K, self.X_augmented, self.S)
        (
            self.final_centers,
            self.final_centers_idx,
            self.final_members,
            _,
        ) = final_clustering.get_centers()
        _, self.post_attack_utility = final_clustering.calculate_fairness_utility(
            self.final_centers_idx
        )
